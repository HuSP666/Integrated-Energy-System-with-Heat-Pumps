...
function calculateOverlapCOP(
	...
	Tm = (or.minTeHigh + or.maxTcLow) / 2  # 默认中间温度
	dT_l = or.midTDifference / 2

	# 高温级计算
	Q_eh, w_h, Q_ch = high_stage(Tm - dT_l, Tc, or.refrigerantHigh.refrigerant, eta_s)
	# 低温级计算
	Q_el, w_l, Q_cl = low_stage(Te, Tm + dT_l, or.refrigerantLow.refrigerant, eta_s)

	# 检查有效性
	if any(isnan, (Q_eh, w_h, Q_ch, Q_el, w_l, Q_cl)) || Q_cl <= 0 || Q_eh <= 0
		return -Inf
	end

	# 总COP计算
	COP = (Q_el * Q_eh) / (Q_el + Q_eh - 1)
	return COP
end
...
高温级制冷循环计算
"""
function high_stage(T_me::Real, T_h::Real, refrigerant::String, η_sh::Real)
	# ...existing code from fu.jl's high_stage function...
end

"""
低温级制冷循环计算
"""
function low_stage(T_e::Real, T_mc::Real, refrigerant::String, η_sl::Real)
	# ...existing code from fu.jl's low_stage function...
end

"""
优化双循环复叠系统的COP
"""
function optimizeOverlapCOP(
	or::OverlapRefrigerant;
	eta_s::Real=0.7,
	dT::Real=0.1,
	step::Real=1.0,
)
	best_COP, best_Te, best_Tc = -Inf, or.refrigerantLow.minTe, or.refrigerantHigh.maxTc

	TeList = collect(or.refrigerantLow.minTe:step:or.minTeHigh)
	TcList = collect(or.maxTcLow:step:or.refrigerantHigh.maxTc)

	for Te in TeList
		for Tc in TcList
			COP = calculateOverlapCOP(or, Te, Tc; eta_s=eta_s, dT=dT)
			if COP > best_COP
				best_COP, best_Te, best_Tc = COP, Te, Tc
			end
		end
	end

	return best_COP, best_Te, best_Tc
end