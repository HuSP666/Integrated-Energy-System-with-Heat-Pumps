...
begin
	...
	validRefrigerants = []
	for refrigerant in [R134a, R1233zdE]
		# 检查临界温度
		critical_temp = CoolProp.PropsSI("Tcrit", refrigerant)
		if TCompressorIn > critical_temp
			println("工质 $(refrigerant) 的临界温度 $(critical_temp) 小于 TCompressorIn $(TCompressorIn)，不适用。")
			continue
		end

		# 检查蒸发温度下的气态压力
		evap_pressure = CoolProp.PropsSI("P", "T", TCompressorIn, "Q", 1, refrigerant)
		if evap_pressure < 0.1e5  # 假设最小气态压力为0.1 bar
			println("工质 $(refrigerant) 在蒸发温度 $(TCompressorIn) 下的气态压力 $(evap_pressure) 太低，不适用。")
			continue
		end

		# 如果通过两次检验，将工质加入有效列表
		push!(validRefrigerants, refrigerant)
		printf("工质 $(validRefrigerants) 通过检验，临界温度: $(critical_temp)，蒸发压力: $(evap_pressure)\n")
	end

	if isempty(validRefrigerants)
		error("没有工质通过检验，请检查输入参数或工质列表。")
	end

	for refrigerant in validRefrigerants
		or = string(refrigerant)*"_Water"
		printf(or)
