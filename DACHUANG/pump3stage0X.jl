...
@everywhere using CoolProp  # 确保每个进程加载必要的模块
@everywhere include("pump3stageX.jl")  # 确保每个进程加载必要的函数
...
useable_refrigerants = ["R23", "R134a", "R404A", "R32"]
"#↑↓时间确实长,尝试用并行计算加速? 此外物态不符的(如Water)暂缺跳出机制"
refrigerants_list = [[r1, r2, r3] for r1 in useable_refrigerants, r2 in useable_refrigerants, r3 in useable_refrigerants]

# 并行优化
results = @distributed (append!) [] for refrigerants in refrigerants_list
    best_cop, (ΔT1_opt, ΔT2_opt) = optimize_three_stage(
        T_low_evap = T_low_evap,
        T_high_cond = T_high_cond,
        refrigerants = refrigerants,
    ...
    [(refrigerants, best_cop, ΔT1_opt, ΔT2_opt)]
end
