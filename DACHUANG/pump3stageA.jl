...
function plot_cop_heatmap(ΔT1_values, ΔT2_values, cop_values)
    heatmap(ΔT1_values, ΔT2_values, cop_values, xlabel="ΔT1 (°C)", ylabel="ΔT2 (°C)", title="COP vs ΔT1 and ΔT2", color=:viridis, clims=(0, maximum(cop_values)), c=:viridis)
    savefig("cop_heatmap.png")
end

function heatStorage_inORout(hourlyTariff,heatStorage,heatStorageCapacity,hour,tag)
    # 【可以优化】添加存储系统的相关计算和逻辑,设放热输出heatStorageSaveRate或1，蓄热输出-1，关闭输出0
...
function pumpWithStorage_dailyCost(hourlyTariff, best_cop, workingHours, heatConsumptionPower,heatStorage,heatStorageCapacity,heatPumpStartTime,heatPumpStopTime)
    total_cost = 0.0
...
function onlyPump_dailyCost(hourlyTariff, best_cop, workingHours,heatDemandPower,heatPumpStartTime,heatPumpStopTime)
    ...
    if workingHours+heatPumpStartTime+heatPumpStopTime>24.0
        ...
        #分段运行模式
        for hour in workingStartHour:workingStartHour+workingHours-1
            total_cost += hourlyTariff[hour] * heatDemandPower / best_cop
        end
...
begin
    ...
    for (i, ΔT1) in enumerate(ΔT1_values)
        for (j, ΔT2) in enumerate(ΔT2_values)
            cop_values[i, j] = three_stage_system(T_low_evap, T_low_evap + ΔT1, T_high_cond - ΔT2, sorted_results[1][1], η_comps, ΔT1, ΔT2)
        end
    end
    ...
    for i in 1:3
        ...
        pWSdC=pumpWithStorage_dailyCost(hourlyTariff, best_cop, workingHours, heatConsumptionPower,heatStorage,heatStorageCapacity,heatPumpStartTime,heatPumpStopTime)
        @printf("因此, ")
